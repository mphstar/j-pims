...
const FormDialog = () => {
    ...
    const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (context.dialog == 'create') {
            ...
            post(route('preference-activity-levels.store'), {
                onSuccess: () => {
                    handleClose();

                    toast({
                        title: 'Success',
                        description: 'Activity level created successfully',
                    });
                },
                onError: (errors) => {
                    toast({
            ...
            post(route('preference-activity-levels.update'), {
                onSuccess: () => {
                    handleClose();
        ...
        >
            <DialogContent className="sm:max-w-lg">
                <DialogHeader className="text-left">
                    <DialogTitle>{`${context.dialog == 'create' ? 'Add' : 'Update'} Activity Level`}</DialogTitle>
                    <DialogDescription>
                        {`${context.dialog == 'create' ? 'Create new' : 'Update'} activity level here. `}
                        Click save when you&apos;re done.
                    </DialogDescription>
                </DialogHeader>
                <ScrollArea className="-mr-4 h-[26.25rem] w-full py-1 pr-4">
                    <form id="activity-level-form" onSubmit={onSubmit} className="space-y-4 p-0.5">
                        ...
                        <div className="grid grid-cols-6 items-center space-y-0 gap-x-4 gap-y-1">
                            ...
                            <Input
                                ...
                                placeholder="Aktivitas ringan, santai"
                                className="col-span-4"
                            />
                            <div className="col-span-6">
                ...
                <DialogFooter>
                    <Button disabled={processing} form="activity-level-form" type="submit">
                        Save changes
                    </Button>
                </DialogFooter>
            </DialogContent>
