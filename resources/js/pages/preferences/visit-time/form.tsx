...
const FormDialog = () => {
    ...
    const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (context.dialog == 'create') {
            ...
            post(route('preference-visit-times.store'), {
                onSuccess: () => {
                    handleClose();

                    toast({
                        title: 'Success',
                        description: 'Visit time created successfully',
                    });
                },
                onError: (errors) => {
                    toast({
            ...
            post(route('preference-visit-times.update'), {
                onSuccess: () => {
                    handleClose();

                    toast({
                        title: 'Success',
                        description: 'Visit time updated successfully',
                    });
                },
                onError: (errors) => {
                    toast({
        ...
        >
            <DialogContent className="sm:max-w-lg">
                <DialogHeader className="text-left">
                    <DialogTitle>{`${context.dialog == 'create' ? 'Add' : 'Update'} Visit Time`}</DialogTitle>
                    <DialogDescription>
                        {`${context.dialog == 'create' ? 'Create new' : 'Update'} visit time here. `}
                        Click save when you&apos;re done.
                    </DialogDescription>
                </DialogHeader>
                <ScrollArea className="-mr-4 h-[26.25rem] w-full py-1 pr-4">
                    <form id="visit-time-form" onSubmit={onSubmit} className="space-y-4 p-0.5">
                ...
                <DialogFooter>
                    <Button disabled={processing} form="visit-time-form" type="submit">
                        Save changes
                    </Button>
                </DialogFooter>
            </DialogContent>
