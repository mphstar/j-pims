...
const FormDialog = () => {
    ...
    const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (context.dialog == 'create') {
            ...
            post(route('preference-destination-types.store'), {
                onSuccess: () => {
                    handleClose();

                    toast({
                        title: 'Success',
                        description: 'Destination type created successfully',
                    });
                },
                onError: (errors) => {
                    toast({
            ...
            post(route('preference-destination-types.update'), {
                onSuccess: () => {
                    handleClose();

                    toast({
                        title: 'Success',
                        description: 'Destination type updated successfully',
                    });
                },
                onError: (errors) => {
                    toast({
        ...
        >
            <DialogContent className="sm:max-w-lg">
                <DialogHeader className="text-left">
                    <DialogTitle>{`${context.dialog == 'create' ? 'Add' : 'Update'} Destination Type`}</DialogTitle>
                    <DialogDescription>
                        {`${context.dialog == 'create' ? 'Create new' : 'Update'} destination type here. `}
                        Click save when you&apos;re done.
                    </DialogDescription>
                </DialogHeader>
                <ScrollArea className="-mr-4 h-[26.25rem] w-full py-1 pr-4">
                    <form id="destination-type-form" onSubmit={onSubmit} className="space-y-4 p-0.5">
                ...
                <DialogFooter>
                    <Button disabled={processing} form="destination-type-form" type="submit">
                        Save changes
                    </Button>
                </DialogFooter>
            </DialogContent>
